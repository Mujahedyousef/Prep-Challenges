...
const oddFiltration = (arr) => {
  ...
  const odd=arr.filter(value=>value%2 != 0);
  return odd;
}
// -------------------------------------------------------------------------------------------------------
...
for(let i=0;i<arr.length;i++){
    if(arr[i].yearsOfExperience>4 && arr[i].tech =="JS"){
        passCandidates.push({fullName:arr[i].firstName +" "+ arr[i].LastName ,tech:arr[i].tech})
    }
}
...
let withoutVowels=arr.filter(check=>{

return !check.match(/[auieo]/g);//If the search value is a string, it is converted to a regular expression.
}) 
...
const skillComparison = (arr1, arr2) => {
   ...
   const skill_1=arr2.filter(skill=> !arr1.includes(skill))
   const skill_2=arr1.filter(skill=> !arr2.includes(skill))
   skill_1.forEach(element => {
    skill_2.push(element);   
   });
   return skill_2;
}
// -------------------------------------------------------------------------------------------------------
